    ...
    if model is None:
        ...
        with col1:
            ...
            * нормируемые уровни H: **100, 300, 1000 А/м**.
            ...
            with st.expander("Связанные стандарты и экспериментальная база"):
                st.markdown("""
                * **МЭК 1000-4-9-93** — международная основа требований к испытанию на устойчивость к импульсному магнитному полю;
                * **ГОСТ 30372-95 / ГОСТ Р 50397-92** — термины и определения в области электромагнитной совместимости технических средств;
                * **ГОСТ Р 50012-92** — методы измерения параметров низкочастотного магнитного поля;
                * **ГОСТ 8.326** — метрологическая аттестация средств измерений;
                * экспериментальная установка З.М. Гизатуллина: генератор импульсного тока, индукционная катушка, измерительные контуры К4 и К5.
                """)
            ...
            **Экспериментальный диапазон H для К4 и К5:**
            * **0,175–1,4 кА/м**.

            Нейросеть выполняет расчет не только в исходных экспериментальных точках,
...
with tab_admin:
    if not st.session_state.admin_logged_in:
        ...
        st.write(
            "Загрузите CSV- или XLSX-выгрузку измерительного стенда. "
            "Обязательные колонки: S, H, V."
        )

        st.markdown("""
        **Формат обучающих данных:**
        * **S** — площадь воспринимающего контура, см²;
        * **H** — напряженность импульсного магнитного поля, кА/м;
        * **V** — амплитуда наведенного напряжения, В.

        Для текущей постановки используются экспериментальные данные по контурам К4 и К5.
        """)
        ...
        st.markdown("""
        Искусственная нейронная сеть обучается на экспериментальных точках,
        но после обучения используется как аппроксимирующая модель зависимости:

        **V = f(S, H)**

        где:
        * **S** — площадь воспринимающего контура;
        * **H** — напряженность импульсного магнитного поля;
        * **V** — амплитуда наведенного напряжения.
        """)
